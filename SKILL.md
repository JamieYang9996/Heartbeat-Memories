...
name: hbm
description: >
  Heartbeat-Memories (HBM) - 完全本地化的长期记忆系统，解决传统Memory文件散乱无序、AI回复冗长无情感的痛点
  新用户常见疑问解答：
  • 冲突？❌ 否，与现有记忆系统完美协作（原系统负责短期记忆，HBM负责长期结构化记忆）
  • 复杂？🚀 一键安装，开箱即用，无需复杂配置
  • 隐私？🔒 100%本地运行，零数据上传，零API调用
  • 资源？📦 轻量级（约100MB磁盘空间，低内存占用）
  • 打扰？⚙️ 心跳回忆智能触发，频率可调，支持静默模式
  • 平台？💻 跨平台支持（Windows WSL/Git Bash、Linux、macOS）
...
### 3. 心跳回忆情感交互（核心创新）
**解决AI回复冗长、缺乏情感的问题，模仿人类对话建立长期连接**：

#### 🎭 高度仿真的人类互动
- **智能触发机制**: AI 像朋友一样主动回忆（例："对了，想起来上周聊的‘海边咖啡厅’‘日落’‘拍照’，你后来去了吗？"）
- **自然对话流**: 随机在日常对话中插入回忆（30%概率），避免机械感
- **情感智能识别**: 分析用户情绪状态，调整互动方式

#### ⚙️ 灵活可调的配置系统  
- **概率可配置**: 每个触发场景的概率可单独调整（30%/50%/100%）
- **频率控制**: 每日上限、最小间隔、节假日特殊规则
- **场景定制**: 支持日常对话、任务完成、目标遗忘、节日关怀等场景
- **敏感日规避**: 自动规避清明节等敏感节日，尊重文化传统

#### 🌱 长期养成与专属感
- **习惯学习**: 记录用户工作模式、偏好话题、常用关键词
- **专属记忆**: 基于历史对话构建个性化回忆库
- **渐进式优化**: 通过静默复盘不断优化触发时机和话术
- **情感进化**: AI 随着时间越来越懂用户，建立真正的“长期关系”

### 4. RAG检索增强系统（v1.1.0 全面升级）
- **混合检索器**: 向量（ChromaDB）+ 关键词（Markdown grep）双通道
- **智能开关系统**: 去重 / Token限制 / 内存缓存，均可动态开关
- **日志自动压缩**: 按月压缩，保留关键信息，压缩比 30%
- **守护进程**: 持久化服务，模型常驻内存，毫秒级响应
...
### RAG检索增强系统（v1.1.0 全面升级）

**RAG 系统已从单一脚本升级为完整模块**（`memory/RAG/`），包含：
- **混合检索器**: 向量 + 关键词双通道检索
- **智能开关系统**: 去重 / Token限制 / 内存缓存，均可动态开启关闭
- **日志自动压缩**: 按月压缩，保留关键信息，压缩比 30%
- **守护进程**: 模型常驻内存，毫秒级响应
